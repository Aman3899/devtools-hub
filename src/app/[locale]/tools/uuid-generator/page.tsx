...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return constructMetadata({
    ...
    canonical: `${siteConfig.url}/${locale}/tools/uuid-generator`,
  });
}
...
export default async function UuidGeneratorPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'tools.uuid-generator' });
  const schema = generateToolSchema('uuid-generator', t('title'), t('description'));
  const breadcrumbSchema = generateBreadcrumbSchema([
    { name: 'Home', item: `${siteConfig.url}/${locale}` },
    { name: t('title'), item: `${siteConfig.url}/${locale}/tools/uuid-generator` },
  ]);
